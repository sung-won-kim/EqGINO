...
class EqSpectralConv(SpectralConv):
    """
    Rotation-Equivariant Grouped Spectral Convolution.
    
    num_groups (int): 
        - 1: Dense Convolution (Original heavy performance)
        - in_channels: Depthwise Separable (Lightest)
        - 2 or 4: Grouped Convolution (Balanced - RECOMMENDED)
    """
    def __init__(self, *args, num_groups=4, **kwargs):
        super().__init__(*args, **kwargs)
        
        # Number of groups (must divide channels)
        assert self.in_channels % num_groups == 0, "in_channels must be divisible by num_groups"
        self.num_groups = num_groups
        self.channels_per_group = self.in_channels // num_groups
        
        if hasattr(self.weight, 'tensor'):
            w_tensor = self.weight.tensor
        else:
            w_tensor = self.weight

        # Redefine weight shape for grouped convolution
        # Shape: (Groups, C_per_group, C_per_group, X, Y, Z)
        # Example: 64 channels, 8 groups -> (8, 8, 8, 30, 30, 30)
        # Parameter count is reduced by ~num_groups vs dense
        grouped_shape = (
            self.num_groups, 
        ...
        with torch.no_grad():
            scale = 1.0 / (grouped_shape[1] * grouped_shape[3] * grouped_shape[4] * grouped_shape[5])
            new_weight = torch.randn(*grouped_shape, dtype=w_tensor.dtype, device=w_tensor.device) * scale
