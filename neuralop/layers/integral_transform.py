...
class IntegralTransform(nn.Module):
    ...
    def forward(self, y, neighbors, x=None, f_y=None, weights=None):
        ...
        self_features = torch.repeat_interleave(x, num_reps, dim=0) # input points
...
class EqIntegralTransform(nn.Module):
    def __init__(
        self,
        channel_mlp=None,
        channel_mlp_layers=None,
    ...
    def forward(self, y, neighbors, x=None, f_y=None, weights=None, pos_embedding_func=None):
        if x is None:
            x = y
        ...
        xy_dist_features = torch.sqrt(torch.pow(rep_features - self_features, 2).sum(dim=-1, keepdim=True))  # relative position

        if pos_embedding_func is not None:
            radial_x = torch.sqrt((self_features**2).sum(dim=-1)).unsqueeze(1) # radial distance of grid points
            radial_x = pos_embedding_func(radial_x)  # (num_total_neighbors, pos_embedding_dim)
            xy_dist_features = pos_embedding_func(xy_dist_features)  # (num_total_neighbors, pos_embedding_dim)

        agg_features = torch.cat([radial_x, xy_dist_features], dim=-1)
        if f_y is not None and (
            self.transform_type == "nonlinear_kernelonly"
            or self.transform_type == "nonlinear"
        ):
